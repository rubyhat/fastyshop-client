...
| Скрипт     | Описание                                                         |
| ---------- | ---------------------------------------------------------------- |
| `dev`      | Запускает проект в режиме разработки на `http://localhost:6300`. |
...
2. **Проверьте локальные переменные окружения**:

   В проекте уже используется `.env.development`. Для backend-интеграции там должен быть указан:

   ```dotenv
   VITE_API_BASE_PATH=http://localhost:6010/api/v1
   ```
